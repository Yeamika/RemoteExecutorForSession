...
use remote_executor_for_session::mcp::EXECUTOR_SESSION_PARAM;
...
async fn main() {
    ...
    let req = |method: &str, mut params: serde_json::Value| {
        if method == "tools/call" {
            if let Some(arguments) = params
                .get_mut("arguments")
                .and_then(|value| value.as_object_mut())
            {
                arguments.insert(EXECUTOR_SESSION_PARAM.to_string(), json!("demo"));
            }
        }
    ...
    let r = ep
        .handle_value(req(
            "tools/call",
            json!({"name":"RemoteExecutorManager","arguments":{
