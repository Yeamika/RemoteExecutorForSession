pub use remote_executor::{
    ...
    read_path, rg_search, stat_path, Caller, ConnectExecutorOptions, ExbashOptions,
    ExbashOutput, ExecutorRequest, ExecutorResponse, FileActionMode, FileActionOptions,
    FileKind, FileStamp as RecFileStamp, GlobOptions, PatchFile, PatchMode, ReadMode,
    ReadOptions, RgOptions, RgOutput, SetDefaultExecutorOptions, ShellManager, StatOptions,
    ToolContext, ToolResult,
};
use serde_json::Value;

pub async fn call_tool(method: &str, params: Value, ctx: &ToolContext) -> anyhow::Result<ToolResult> {
    dispatch_tool(method, params, ctx).await
}

pub async fn call_file_action(options: FileActionOptions, ctx: &ToolContext) -> anyhow::Result<ToolResult> {
