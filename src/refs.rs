...
pub fn file_key_ref(executor: Option<&str>, file_key: &str) -> String {
    format!("{}:{}", executor.map(str::trim).filter(|item| !item.is_empty()).unwrap_or("local"), file_key)
}
...
pub fn parse_hash_ref(target: &str) -> Option<HashRefTarget> {
    ...
    Some(HashRefTarget { filename: filename.to_string(), small_hash_code: code.to_uppercase() })
}

pub fn make_entry_parts(executor: Option<&str>, file_key: &str, file_path: &str, hash_code: &str) -> (String, String, String, String) {
...
pub fn inject_file_ref(args: &Value, entry: Option<&FileRefEntry>) -> FileRefInjection {
    ...
    if let Some(entry) = entry {
        object.insert("filePath".to_string(), Value::String(entry.file_path.clone()));
        object.insert("hashCheckMode".to_string(), Value::Bool(true));
        object.insert("hashCode".to_string(), Value::String(entry.hash_code.clone()));
        if entry.executor == "local" {
            object.remove("executor");
            object.remove("targetExecutor");
        } else {
            object.insert("targetExecutor".to_string(), Value::String(entry.executor.clone()));
            object.remove("executor");
        }
    }

    FileRefInjection { args: Value::Object(object), executor }
}

pub fn extract_file_ref_update(result: &RecToolResult, executor: impl Into<String>) -> Option<FileRefUpdate> {
    ...
    Some(FileRefUpdate { executor: executor.into(), file, hash_code })
}
...
mod tests {
    ...
    fn computes_opencode_file_key_ref_parts() {
        let (file_key_ref, filename, small, label) = make_entry_parts(Some("box"), "file-key", "/remote/path/App.ts", "sha256:abc");
